...
// Remove instance of image from jugglingDB and Cloudinary.
function delete_image(req, res) {
	var public_id = req.body.public_id;
	Photo.all().then(function(photos) {
		for(i in photos) {
			if (photos[i].image.public_id == public_id) {
				photos[i].destroy(function (err) {
					console.log(err);
				});
			}
		}
...
function create_through_server(req, res) {
  ...
  cloudinary.uploader.upload(imageFile, 
	  ...
	  { tags: ['express_sample'].concat(req.body.tag),
		  width: 400, 
		  height:400
  		})
    .then(function (image) {
...
module.exports.wire = function (app) {
  ...
  app.post('/photos/delete_image', delete_image);
